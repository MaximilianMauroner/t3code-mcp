...
For a new assignment, prefer `t3_task_start`. It sends one recoverable bootstrap containing thread creation and the required first instruction, and requires an explicit `runtimeMode`. By default it uses the project's current checkout (or an explicitly supplied existing `worktreePath`). Set `workspaceMode: "worktree"` and supply `branch` as the base branch to ask T3 to create a managed worktree before starting the turn. `startFromOrigin: true` fetches and resolves that branch from `origin`; `false` or omission starts from the local branch. T3 chooses the new worktree path and task branch, so `worktreePath` is invalid in this mode. Worktree bootstrap also runs the project's configured setup script. This requires a T3 server whose HTTP orchestration route supports `thread.turn.start.bootstrap`.

One parent idempotency key owns the deterministic bootstrap receipt; retries with identical input reconcile the same thread, message, and run. New tasks use `dispatch_rejected`, `dispatch_uncertain`, `run_accepted`, and `rejected`; `thread_create_uncertain` and `thread_created` remain readable for task records created by older two-command gateway versions.
