...
| “Create an empty thread without starting work.” | `t3_thread_create` |
...
For a new assignment, prefer `t3_task_start`. It performs thread creation and first-turn dispatch as a recoverable two-command sequence and requires an explicit `runtimeMode`. By default it uses the project's current checkout (or the explicitly supplied legacy `branch`/`worktreePath`). Set `workspaceMode: "worktree"` and supply `branch` as the base branch to ask T3 to create a managed worktree before starting the turn. `startFromOrigin: true` fetches and resolves that branch from `origin`; `false` or omission starts from the local branch. T3 chooses the new worktree path and task branch, so `worktreePath` is invalid in this mode. Worktree bootstrap also runs the project's configured setup script. This requires a T3 server whose HTTP orchestration route supports `thread.turn.start.bootstrap`.

One parent idempotency key owns deterministic child keys; retries with identical input reconcile the same thread, message, and run. The gateway never sends while thread creation is uncertain. Its stages distinguish `thread_create_uncertain`, `thread_created`, `dispatch_rejected`, `dispatch_uncertain`, `run_accepted`, and `rejected`.

The task journal stores the title, payload hash, stage, IDs, optional Git baseline, and timestamps, but not the instruction text. If the gateway stops after creating the thread but before proven dispatch, `t3_task_get` safely identifies the partial assignment; continuing it requires retrying `t3_task_start` with the same key and identical original input. `t3_task_get` reconciles uncertain child operations without replaying them. `t3_tasks_list` is bounded and deterministic; use task detail for fresh thread/run state.
...
The low-level thread tools remain available. Call `t3_providers_list` when a project has no default model. To continue existing work, send another message once its thread is idle. Busy threads return `thread_busy`; uncertain low-level dispatches return a durable operation handle. Creating a thread alone does not start a turn, and its legacy runtime default remains unchanged.

Failed run, thread, and overview records include a structured `failure` when T3 exposes a session error. The pinned upstream contract currently provides `lastError`, provider identity, and a turn link, so those fields are propagated and sanitized. Category/code/reset/retry metadata is only returned when upstream explicitly supplies it; a 429-looking message is never classified by text or status alone. `category: "unknown"` and null reset fields are intentional when the source is less specific. A failure is never attached to a different run, and a previous turn's assistant message is not returned as the failed run's response.
